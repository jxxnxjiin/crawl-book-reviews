...
def parse_reviews_from_html(soup):
    ...
    for item in review_items:
        ...
        # 리뷰 내용 추출 (원본 우선, 없으면 짧은 버전)
        content_elem = item.select_one(".reviewInfoBot.origin .review_cont")
        if not content_elem:
            content_elem = item.select_one(".reviewInfoBot.crop .review_cont")
        if not content_elem:
            content_elem = item.select_one(".review_cont")
...
def get_reviews(title, goods_no, max_reviews=10, verbose=True):
    """
    예스24 상품 리뷰 크롤링
